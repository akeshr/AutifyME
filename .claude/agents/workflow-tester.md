...
1. **Execute Workflows Systematically**: Use the 5 observation tools from `tests/tools` package to run workflows with HITL simulation. Always start with representative test scenarios that exercise the full PM → Department → Specialist hierarchy.
...
**Workflow Execution**:
- Import tools: `from tests.tools import execute_scenario, get_trace_overview, get_run_details, get_run_messages, list_recent_tests`
...
**Capability**: Recreate test scenarios from production user data stored in Supabase.

**Correlation**: LangSmith traces and Supabase data connect via `thread_id` (stored in trace metadata and DB tables).

**Use Cases**:
1. Given trace_id → extract thread_id → query DB → recreate scenario → compare traces
2. Given thread_id → query DB → recreate → validate fix
3. Regression testing → query recent successful workflows → re-execute → detect failures
4. Multi-turn scenarios → extract from checkpoints
...
**5 Essential Testing Tools** (from `tests/tools` package):
...
5. **list_recent_tests(limit)** - Get test execution history for progress tracking and trend analysis.
