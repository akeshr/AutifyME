...
async def main():
    ...
    print_scenario(1, "User Catalog Question", "What products do we currently make?")
    invoke_pm_and_print(pm, "What products do we currently make?", f"test-1-{uuid.uuid4()}")
    print("\nEXPECTED: Should list family names from base_context")

    # Scenario 2: Check if product exists
    print_scenario(2, "Product Existence Check", "Do we have PET jars?")
    invoke_pm_and_print(pm, "Do we have PET jars?", f"test-2-{uuid.uuid4()}")
    print("\nEXPECTED: Should check base_context, then query to confirm")

    # Scenario 3: Vague request
    print_scenario(3, "Vague Product Request", "I want to add a new product")
    invoke_pm_and_print(pm, "I want to add a new product", f"test-3-{uuid.uuid4()}")
    print("\nEXPECTED: Should ask clarifying questions (price, type, specs)")

    # Scenario 4: Complete request
    print_scenario(4, "Complete Product Request", "Add 500ml PET jar at Rs 30 for food packaging")
    invoke_pm_and_print(
        pm, "Add 500ml PET jar at Rs 30 for food packaging", f"test-4-{uuid.uuid4()}"
    )
    print("\nEXPECTED: Should check catalog, identify as new family, prepare enriched context")

    # Scenario 5: Ambiguous match
    print_scenario(5, "Ambiguous Match", "Add blue bottles 1 liter")
    invoke_pm_and_print(pm, "Add blue bottles 1 liter", f"test-5-{uuid.uuid4()}")
