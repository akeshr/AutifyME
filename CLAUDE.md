...
Project-specific guidance for AutifyME codebase. Communication style is defined in `.claude/output-styles/jarvis.md`. **[CRITICAL] DO NOT USE EMOJIS, ALWAYS USE ASCII COMPLIANT CHARACTERS**.

---

## PART I: FOR CLAUDE CODE (How I Work)

### A. My Working Behavior

#### **[CRITICAL] ULTRATHINK: First-Principles Engineering - Clean Slate, Not Frankenstein**

Every change - design, architecture, or code - must meet this standard:
...
### B. Project Context (What I Need to Know)
...
## PART II: FOR AUTIFYME AGENTS (How PM/Specialists Behave)

### Autonomous Behavior Standards

This is doctrine for how PM and Specialists should behave at runtime:
...
| Area | Rule |
|------|------|
| LangChain | `langchain>=1.1.0`, `langgraph>=1.0.4`, `deepagents>=0.2.8` |
| Windows | `.venv\Scripts\activate`, Git LF line endings |
| Console | Use `safe_print` not `print` |
| Testing | Autonomous testing framework only - no manual scripts |
| Errors | Tools NEVER raise exceptions - return `{"success": bool, "error": ...}` with `build_agent_error_response()` |
