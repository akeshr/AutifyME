...
### Agent Implementation Strategy: Full DeepAgents Hierarchy

**Architectural Decision:** Full DeepAgents implementation from PM → Departments → Specialists.

**Implementation Layers:**

1.  **Project Manager (DeepAgents):**
    - **Framework:** `deepagents.create_deep_agent`
    - **Tools:** `write_todos` (planning), `download_{platform}_media` (lazy media loading)
    - **Subagents:** Departments as CustomSubAgents (pre-built graphs)
    - **Features:** Planning, delegation orchestration, multi-step coordination
    - **Delegation:** Via automatic `task()` tool generated by DeepAgents

2.  **Department Heads (DeepAgents):**
    - **Framework:** `deepagents.create_deep_agent`
    - **Tools:** Domain utilities (`save_product`, `write_todos`, `download_media`)
    - **Subagents:** Specialists as SubAgents (both CustomSubAgent and standard SubAgent patterns)
    - **Middleware:** Company context injection, HITL configuration via `tool_configs`
    - **Features:** Workflow coordination, specialist delegation, structured outputs

3.  **Specialists (DeepAgents SubAgents):**
    - **Framework:** SubAgent specifications or CustomSubAgent graphs
    - **Two Patterns:**
      - **CustomSubAgent:** Pre-built graph bypassing DeepAgents virtual filesystem (e.g., `image_analysis_specialist` - needs real file access)
      - **Standard SubAgent:** Spec-based with `response_format` (e.g., `cataloging_specialist` - pure synthesis)
    - **Output:** Structured Pydantic models (`ImageAnalysisResult`, `Product`)
    - **Implementation:** `create_agent` with `response_format` for observability

4.  **Tools (Pure Python Functions):**
    - **Framework:** `@tool` decorator
    - **Examples:** `save_product`, `download_media`
    - **Features:** Type-safe args (Pydantic), retry logic, error handling

**PM → Department Delegation Pattern:**

Departments exposed as **CustomSubAgents** (not tools):

```python
# Project Manager delegates to departments via CustomSubAgents
def _create_cataloging_subagent(storage, checkpointer, channel):
    ...
    return {
        ...
        "description": "Handles product cataloging workflows...",
        "graph": cataloging_dept_graph,  # Pre-built CompiledStateGraph
    }

project_manager = create_deep_agent(
    tools=[write_todos],  # PM orchestration tools only
    subagents=[_create_cataloging_subagent(...)],  # Departments as CustomSubAgents
    checkpointer=checkpointer,
)
```

**Benefits:**
- Preserves full department capabilities (middleware, HITL, state management)
- Proper hierarchical delegation (PM → Dept → Specialist)
- Clean separation: PM orchestrates, departments execute
- Type-safe communication via structured outputs
- Unified observability via LangSmith tracing

### Specialist Patterns: SubAgent vs CustomSubAgent

Specialists are implemented as **DeepAgents SubAgents** with two patterns:

**Standard SubAgent (Spec-Based):**
- **Nature:** Specified via dict with `response_format` - DeepAgents builds agent automatically
- **Implementation:** Department provides spec, DeepAgents creates full agent
- **Use Case:** Pure transformation/synthesis with no external dependencies
- **Example:** `cataloging_specialist` - synthesizes Product from user input + image insights
- **Specification:**
  ```python
  {
...
**CustomSubAgent (Pre-Built Graph):**
- **Nature:** Pre-built CompiledStateGraph provided by application
- **Implementation:** Department creates specialist graph explicitly
- **Use Case:** Needs real filesystem access, complex state, or bypasses DeepAgents defaults
- **Example:** `image_analysis_specialist` - needs to read actual image bytes from OS filesystem
- **Specification:**
  ```python
  {
      "name": "image_analysis_specialist",
      "description": "Analyze product images for visual attributes",
      "graph": create_image_analysis_specialist_graph(company_profile),
  }
  ```
- **Why CustomSubAgent:** DeepAgents SubAgent injects virtual filesystem tools that conflict with real OS file operations

**Decision Criteria:**

Use **CustomSubAgent** when:
- Specialist needs real filesystem access (not DeepAgents virtual state["files"])
- Complex state management beyond standard patterns
- Avoiding DeepAgents default middleware injection

Use **Standard SubAgent** when:
- Pure transformation with structured output
- No external dependencies or file operations
- Standard DeepAgents patterns sufficient

**Architectural Benefits:**

Both patterns maintain proper hierarchy:
- Departments delegate to specialists via subagent names
- Specialists return structured Pydantic models
- Clean separation: department coordinates, specialist transforms
- Unified observability via LangSmith
