...
- workflows/project_manager.py builds the DeepAgents PM, loading PMBaseContext via middleware/context_middleware.py and attaching SubAgents from specialists/ (catalog_specialist.py, creative_specialist.py).
...
**Data & Tools**
- tools/data_engine/ is the canonical data access layer: write_data.py, read_data.py, inspect_schema.py, aggregate_data.py handle all database operations with validation and HITL integration.
- Schema discovery flows through schemas/registry/schema_models.py and tools/data_engine/inspect_schema.py; use them before inventing bespoke SQL.
...
**Specialists & Prompts**
- specialists/*.py follow create_*_specialist factories returning SubAgent dicts; mirror catalog_specialist.py or creative_specialist.py when adding domains.
...
- Keep tool HITL policies inside specialist tool_configs (see catalog_specialist.py) so runner stays generic.
