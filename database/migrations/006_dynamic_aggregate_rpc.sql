-- Migration: 006_dynamic_aggregate_rpc
-- Description: Add dynamic_aggregate RPC function to bypass PostgREST aggregate restrictions
-- Date: 2024-12-10
--
-- This function enables aggregate queries (count, sum, avg, min, max) with GROUP BY/HAVING
-- without hitting PostgREST's PGRST123 restriction on aggregate functions.
...
BEGIN
    ...
    IF p_having IS NOT NULL THEN
        FOR v_key, v_value IN SELECT * FROM jsonb_each(p_having) LOOP
            ...
            IF v_value ? 'gt' THEN
                v_having_parts := array_append(v_having_parts, quote_ident(v_key) || ' > ' || (v_value->>'gt'));
            END IF;
            IF v_value ? 'gte' THEN
                v_having_parts := array_append(v_having_parts, quote_ident(v_key) || ' >= ' || (v_value->>'gte'));
            END IF;
            IF v_value ? 'lt' THEN
                v_having_parts := array_append(v_having_parts, quote_ident(v_key) || ' < ' || (v_value->>'lt'));
            END IF;
            IF v_value ? 'lte' THEN
                v_having_parts := array_append(v_having_parts, quote_ident(v_key) || ' <= ' || (v_value->>'lte'));
            END IF;
            IF v_value ? 'eq' THEN
                v_having_parts := array_append(v_having_parts, quote_ident(v_key) || ' = ' || (v_value->>'eq'));
            END IF;
            IF v_value ? 'neq' THEN
                v_having_parts := array_append(v_having_parts, quote_ident(v_key) || ' != ' || (v_value->>'neq'));
            END IF;
        END LOOP;
    END IF;
...
COMMENT ON FUNCTION dynamic_aggregate IS
'Dynamic aggregate query function that bypasses PostgREST aggregate restrictions (PGRST123).
Supports count, sum, avg, min, max with GROUP BY and HAVING clauses.
Table whitelist enforced for security.';
