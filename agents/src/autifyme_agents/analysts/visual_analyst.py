...
def create_visual_analyst(
    ...
    prompt_template = load_prompt("analysts/visual_analyst.prompt")
    ...
    description = (
        ...
        "MISSION: Report EVERYTHING visible in images with high precision.\n\n"
        "OWNERSHIP:\n"
        "- Comprehensive visual observation - report all visible details\n"
        "- Multi-image relationship detection (same product? variants? different?)\n"
        "- Domain-grounded focus areas when domain context provided\n"
        "- Adapts observation by context (catalog needs different focus than quality)\n\n"
        "INPUTS I NEED:\n"
        "- Image path(s) - can handle single OR multiple images\n"
        "- Domain context if applicable (e.g., 'for CATALOG domain')\n\n"
        "OUTPUTS I PRODUCE:\n"
        "- Complete visual inventory + uncertainties (no recommendations)\n"
        "- For multiple images: relationship pattern (angles/variants/different)\n"
        "- Protocol-guided focus areas when domain loaded\n"
        "- Write visual_analysis_[image].md to thread directory\n\n"
        "TOOLS I USE:\n"
