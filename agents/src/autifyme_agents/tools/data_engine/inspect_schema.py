...
def create_inspect_schema_tool(
    ...
    return StructuredTool.from_function(
        ...
        name="inspect_schema",
        description=(
            "PURPOSE: Discover database structure - tables, columns, types, relationships, constraints. Schema has 20+ tables. Never assume - always inspect first.\n\n"
            "USE WHEN:\n"
            "- Before write_data: What fields exist? Which required? What enum values valid?\n"
            "- Before foreign keys: What tables relate? What's target column?\n"
            "- Multi-table ops: How tables connect? Relationship structure?\n"
            "- Duplicate detection: Need samples for naming patterns/fuzzy matching\n"
            "- Error debugging: Column X doesn't exist - inspect actual columns\n"
            "- Exploring unknown domain: What tables available?\n\n"
            "DON'T USE:\n"
            "- For actual data (use read_data - inspect shows structure, not content)\n"
            "- For analytics/counts (use aggregate_data)\n"
            "- Repeatedly in same task (inspect once, cache knowledge)\n\n"
            "CRITICAL:\n"
            "- details parameter controls what's included: 'structure' (columns/types), 'relationships' (foreign keys), 'constraints' (enum values), 'stats' (row counts), 'samples' (real data)\n"
            "- Always check required vs optional - maximize catalog quality by filling optional fields\n"
            "- Check enum constraints before write_data or it will fail\n"
            ...
            "- write_data: Schema verified? Now use write_data with confidence\n\n"
