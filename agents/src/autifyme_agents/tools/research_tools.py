...
def create_research_product_tool(settings: Settings) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        ...
        description=(
            "Research product information using web search (Tavily API). "
            "Use to enrich minimal product data, validate specifications, research pricing, "
            "verify brands/manufacturers, or find technical documentation. "
            "Returns structured ProductResearchResult with AI-generated answer, ranked sources, "
            "confidence scores, and key findings. "
            "Cost: ~1 credit per search (~150/month expected, within 1000 free tier). "
...
def create_extract_web_content_tool(settings: Settings) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        ...
        description=(
            "Extract and parse web page content for LLM analysis (Tavily Extract API). "
            "Use for deep-dive into specific URLs from research results: product pages, spec sheets, "
            "technical documentation, manufacturer websites. "
            "Returns structured WebContentAnalysis with markdown/text content, metadata, structured data. "
            "Cost: ~1 credit per extraction. Use selectively for high-value sources only. "
