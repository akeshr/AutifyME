...
def create_research_product_tool(settings: Settings) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        ...
        description=(
            ...
            "ALSO CONSIDER:\n"
            "- extract_web_content_tool: Source has 0.8+ relevance? Deep-dive for full content\n"
            "- read_data: Research complete? Check catalog for similar products\n"
            "- write_data: Research + catalog check done? Ready to create product\n\n"
...
def create_extract_web_content_tool(settings: Settings) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        ...
        description=(
            ...
            "ALSO CONSIDER:\n"
            "- research_product_tool: Start here first for broad search, then extract high-value URLs\n"
            "- Pattern: research_product_tool (broad) -> extract_web_content_tool (deep on 0.8+ sources)\n\n"
            "RETURNS: Always a structured dict with success flag.\n"
