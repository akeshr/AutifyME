...
def create_image_studio_tool(storage: StorageUploader | None = None) -> StructuredTool:
    ...
    return StructuredTool.from_function(
        ...
        name="image_studio",
        description=(
            "Professional image processing with Gemini 3 Pro Image. Transform uploads into catalog-ready assets.\n\n"
            "LABELED IMAGES: Each image has a label you reference in specs.\n"
            "  images=[{path: 'inbox/photo.jpg', label: 'source'}]\n"
            "  Then use [source] in specs: extraction={target_description: 'jar in [source]'}\n\n"
            "CORE SPECS (use what applies):\n"
            "- fidelity: CRITICAL - preserve product identity {preserve_colors, preserve_artwork, hero_features}\n"
            "- extraction: isolate product {target_description, isolation, edge_treatment}\n"
            "- background: {treatment: 'transparent'|'solid_color'|'scene', color}\n"
            "- lighting, composition, material_treatment, scene, placement, enhancement\n"
            "- output: {format: 'PNG', size: '2K', aspect_ratio: '1:1'}\n\n"
            "WORKFLOW: view_image (assess) -> image_studio (process) -> view_image (verify)\n\n"
            "PROTOCOL: Load 'image_studio' protocol for detailed patterns, anti-patterns, and examples.\n\n"
            "RETURNS: {success, storage_path, metadata} + visual preview of generated image."
        ),
